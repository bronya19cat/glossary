...
# 创建根窗口
root = tk.Tk()
root.title("English Terms Glossary (v0.3) : " + str(len(glossary)))  # 修改标题，添加术语计数
...
glossary_frame.place(x=5, y=20, width=100, height=380)
...
# 创建一个框架用于容纳搜索和添加术语相关的部件
search_frame = tk.Frame(root)
search_frame.place(x=115, y=20, width=280, height=380)
...
label_full_form = tk.Label(search_frame, textvariable=full_form, wraplength=280)  # 创建一个标签用于显示术语的全称，并设置换行宽度为200
label_full_form.pack()

explanation = tk.StringVar()
label_explanation = tk.Label(search_frame, textvariable=explanation, wraplength=280)  # 创建一个标签用于显示术语的解释
label_explanation.pack()

# Add New Term模块
button_add = tk.Button(search_frame, text="Add", command=add_term)  # 创建一个按钮部件，并关联一个函数作为回调函数
button_add.pack(side=tk.BOTTOM)

entry_add_expl = tk.Entry(search_frame)  # 创建一个文本输入框部件
entry_add_expl.pack(side=tk.BOTTOM)

entry_add_full = tk.Entry(search_frame)  # 创建一个文本输入框部件
entry_add_full.pack(side=tk.BOTTOM)

entry_add_term = tk.Entry(search_frame)  # 创建一个文本输入框部件
entry_add_term.pack(side=tk.BOTTOM)

label_add = tk.Label(search_frame, text="Add New Term")  # 创建一个标签用于显示文本
label_add.pack(side=tk.BOTTOM)
