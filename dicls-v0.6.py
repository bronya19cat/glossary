...
Version:
- 0.6 Added networking collection function
"""
...
# 上传文件到坚果云
def upload_to_jianguoyun(filename):
    ip_address = get_ip()
    ...
    # 上传文件到指定路径
    to_path = '/Nutshare/glossaries/{filename}'.format(filename=change_filename)  # 替换为您的实际目标路径
    client.upload_file(from_path=filename, to_path=to_path, overwrite=True)
    print(f"File uploaded successfully: {filename}")

# 上传文件
upload_to_jianguoyun('glossary.json')
...
### 创建GUI
root = tk.Tk()  # 创建根窗口
root.title("Glossary (v0.6) : " + str(len(glossary)) + " terms in glossary.json")  # 修改标题，添加术语计数
...
label_full_form = tk.Label(show_frame, textvariable=full_form, wraplength=280)  # 创建一个标签用于显示术语的全称，并设置换行宽度
...
## 底部Add New Term模块
b_frame = tk.Frame(root)  # 创建一个框架用于容纳搜索术语相关的部件
b_frame.place(x=120, y=270, width=260, height=120)

# 左下输入框
lb_frame = tk.Frame(b_frame)
...
button_readme = tk.Button(rb_frame, text="Readme", command=open_readme, width=10)  # README按钮
button_readme.pack()
