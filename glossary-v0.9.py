...
- 0.7 Turn off auto-upload, enabling manual uploads.
- 0.8 Replaced 'label' by 'text' in 'show_frame', failed to add link function in text of 'explanation'. --240308
- 0.9 Added a new function: we can see the term's location in the left list, after searching for it. --240313
...
# 创建GUI
root = tk.Tk()  # 创建根窗口
root.title(f"Glossary ({current_version}) - {len(glossary)} terms")  # 修改标题，添加术语计数
...
search_frame.place(x=120, y=10, width=260, height=40)
...
show_frame.place(x=120, y=60, width=260, height=170)

# 移除 StringVar，不再需要
text_full_form = tk.Text(show_frame, wrap=tk.WORD, height=2)  # 移除 textvariable 参数
text_full_form.pack()

text_explanation = tk.Text(show_frame, wrap=tk.WORD, height=10)  # 移除 textvariable 参数
text_explanation.pack()

# 第三层：Add New Term模块
add_frame = tk.Frame(root)  # 创建一个框架用于容纳搜索术语相关的部件
add_frame.place(x=120, y=240, width=260, height=100)
...
bottom_frame.place(x=120, y=350, width=260, height=40)
